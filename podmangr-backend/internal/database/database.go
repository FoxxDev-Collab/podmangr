...
var migrations = []migration{
	...
	// Phase: Database Management
	{
		...
		up: `
			CREATE TABLE managed_databases (
				...
				admin_user TEXT,
				admin_password TEXT,
				network TEXT DEFAULT 'podmangr-data',
				volume_name TEXT,
				status TEXT DEFAULT 'stopped',
	...
	// Phase: Two-Tier Database Management (Database Servers + Databases)
	{
		name: "027_create_two_tier_databases",
		up: `
			-- Database Servers (Tier 1 - Podman containers running DB engines)
			CREATE TABLE database_servers (
				id TEXT PRIMARY KEY,
				container_id TEXT NOT NULL,
				name TEXT NOT NULL UNIQUE,
				engine TEXT NOT NULL CHECK(engine IN ('postgresql', 'mariadb', 'mysql')),
				version TEXT NOT NULL,
				image TEXT NOT NULL,
				network TEXT NOT NULL DEFAULT 'podmangr-db',
				status TEXT DEFAULT 'stopped' CHECK(status IN ('running', 'stopped', 'starting', 'stopping', 'error', 'unknown')),
			...
			CREATE INDEX idx_database_servers_name ON database_servers(name);
			CREATE INDEX idx_database_servers_engine ON database_servers(engine);
			CREATE INDEX idx_database_servers_status ON database_servers(status);
			CREATE INDEX idx_database_servers_container ON database_servers(container_id);

			-- Databases within servers (Tier 2 - Logical databases)
			CREATE TABLE databases (
				id TEXT PRIMARY KEY,
				server_id TEXT NOT NULL REFERENCES database_servers(id) ON DELETE CASCADE,
				name TEXT NOT NULL,
				username TEXT NOT NULL,
			...
			INSERT OR IGNORE INTO settings (key, value) VALUES
				('database.default_network', 'podmangr-db');
		`,
	},
	{
		name: "028_create_secrets_store",
		up: `
			-- Centralized secrets store for all containers
			CREATE TABLE secrets (
				id TEXT PRIMARY KEY,
				container_id TEXT,                    -- Optional: link to a container
				container_name TEXT,                  -- Container name for display (in case container is deleted)
				name TEXT NOT NULL,                   -- Secret name/label (e.g., "Root Password", "API Key")
				value_encrypted TEXT NOT NULL,        -- AES-256-GCM encrypted value
				secret_type TEXT NOT NULL DEFAULT 'password' CHECK(secret_type IN ('password', 'api_key', 'connection_string', 'env_var', 'certificate', 'other')),
				category TEXT,                        -- Optional grouping (e.g., "database", "api", "service")
				description TEXT,                     -- Optional description
				metadata TEXT,                        -- JSON metadata (port, host, username, etc.)
				created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
				updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
				created_by INTEGER REFERENCES users(id) ON DELETE SET NULL
			);
			CREATE INDEX idx_secrets_container_id ON secrets(container_id);
			CREATE INDEX idx_secrets_container_name ON secrets(container_name);
