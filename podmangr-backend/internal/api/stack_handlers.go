...
const stacksBaseDir = "/var/lib/podmangr/stacks"
...
func deployStackHandler(c echo.Context) error {
	...
	// Verify stack path and compose file exist
	composePath := filepath.Join(stack.Path, "docker-compose.yml")
	if _, err := os.Stat(composePath); os.IsNotExist(err) {
		// Try to recreate the compose file from database content
		if stack.ComposeContent != "" {
			if err := os.MkdirAll(stack.Path, 0755); err != nil {
				return c.JSON(http.StatusInternalServerError, map[string]string{
					"error": "Failed to create stack directory: " + err.Error(),
				})
			}
			if err := writeComposeFiles(stack.Path, stack.ComposeContent, stack.EnvContent); err != nil {
				return c.JSON(http.StatusInternalServerError, map[string]string{
					"error": "Failed to write compose files: " + err.Error(),
				})
			}
		} else {
			return c.JSON(http.StatusBadRequest, map[string]string{
				"error": "Compose file not found and no content stored in database",
			})
		}
	}
