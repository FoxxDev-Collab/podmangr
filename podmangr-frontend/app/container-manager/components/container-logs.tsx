...
export function ContainerLogs({ containerId, containerName, onClose }: ContainerLogsProps) {
  ...
  const parsedLogs = useMemo((): ParsedLog[] => {
    return logs.map((log) => {
      ...
      // Try to detect timestamp at start
  ...
  return (
    <Dialog open={!!containerId} onOpenChange={onClose}>
      <DialogContent
        className={`${fullscreen ? "max-w-full h-screen" : "max-w-6xl h-[800px]"} p-0 gap-0 flex flex-col`}
      >
        <TooltipProvider>
          <DialogHeader className="px-4 py-3 border-b border-border/50 flex-row items-center justify-between space-y-0">
            <DialogTitle className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                {containerName && (
                  <span className="text-sm text-muted-foreground font-normal">{containerName}</span>
                )}
              </div>
            </DialogTitle>
            <div className="flex items-center gap-1.5">
              ...
              <Button variant="ghost" size="icon" onClick={() => setFullscreen(!fullscreen)} className="h-8 w-8 ml-2">
          ...
          <div className="px-4 py-3 border-b border-border/50 bg-muted/30 space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex-1 min-w-[200px] relative">
                ...
                <Tooltip>
                  ...
                  <TooltipContent>{useRegex ? "Regex mode enabled" : "Enable regex search"}</TooltipContent>
                </Tooltip>
              </div>
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <Switch id="auto-scroll-dialog" checked={autoScroll} onCheckedChange={setAutoScroll} />
                <Label htmlFor="auto-scroll-dialog" className="text-sm cursor-pointer">Auto-scroll</Label>
              </div>
              <div className="flex items-center gap-2">
                <Switch id="timestamps-dialog" checked={showTimestamps} onCheckedChange={(v) => { setShowTimestamps(v); handleRefresh(); }} />
                <Label htmlFor="timestamps-dialog" className="text-sm cursor-pointer">Timestamps</Label>
              </div>
              <div className="flex items-center gap-2">
                <Switch id="wrap-dialog" checked={wrapLines} onCheckedChange={setWrapLines} />
                <Label htmlFor="wrap-dialog" className="text-sm cursor-pointer flex items-center gap-1">
                ...
                {filteredLogs.map((log, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="w-5 flex-shrink-0 py-0.5">{getLevelIcon(log.level)}</span>
          ...
          <div className="px-4 py-2 border-t border-border/50 bg-muted/30 text-xs text-muted-foreground flex items-center justify-between">
            <span>{following ? "Streaming live logs..." : `Showing ${filteredLogs.length} of ${logs.length} lines`}</span>
            <span className="text-muted-foreground/60">Ctrl+F to search • Esc to clear</span>
          </div>
        </TooltipProvider>
      </DialogContent>
    </Dialog>
